...
###Model###    
for i in  range(5):
  model = tf.keras.applications.EfficientNetB3(input_shape=(224, 224, 3),include_top=False,weights='imagenet')
  ...
  model.save_weights('EfficientNetB3_'+str(i+1)+'.h5')
...
crp_reports=[]
for i in range(5):
  model.load_weights('EfficientNetB3_'+str(i+1)+'.h5')
...
repdf.to_csv("EfficientNetB3_.csv")
